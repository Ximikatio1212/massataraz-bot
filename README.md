...
### UX: одиночное «якорное» меню (canvas)

Бот использует один интерактивный экран-меню на пользователя. Вся текстовая навигация (каталог, корзина, админ-панель, оформление) редактирует его на месте — дубликатов меню в чате не появляется. Фотографии товаров/курсов показываются отдельным медиа-сообщением и убираются при переходе назад, так что в чате всегда видно только актуальное меню и недавний контент.
...
src/
  bot/
    ...
    helpers.ts      — утилиты отправки сообщений, canvas-рендер (якорное меню)
