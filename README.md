...
### 🤖 ИИ-консультант (опционально)

- **Клиент**: кнопка «💬 Вопрос консультанту» включает диалог. ИИ отвечает по реальному каталогу (наличие, описание, цены тенге), рекомендует товары/связки, проверяет свой заказ и кратко (2–3 предложения) объясняет вещества (тестостерон, креатин и т.п.) через Wikipedia. Только чтение — создавать ничего не может.
- **Админ**: любой текст без активного сценария уходит ассистенту. Он создаёт категории/товары/курсы, наполняет связки, меняет остатки — всегда после уточнения деталей и явного подтверждения «да».
- **Провайдеры**: Gemini (по умолчанию, лучшее бесплатное качество), Mistral, Groq, OpenRouter. Переключается одной переменной `AI_PROVIDER`. Все OpenAI-совместимы, function calling включён.
- **Память**: последние ~20 сообщений диалога хранятся per-user в базе (не теряются между вызовами serverless-функций).
- Без настроенного ключа бот работает как раньше (без ИИ).
...
- Интерфейс на русском/казахском языках (кнопка «🌐 Тіл» в главном меню)
